...
## 🎉 ОБНОВЛЕНИЕ 2025-01-09

**✅ Новый flow создания ботов!**
...
- **Логи**: Middleware доступа к ПДн (`pd_access_log.py`) логирует только **endpoint, method, user_id, timestamp**. Тела запросов (body), текст сообщений и chat_id в лог **не попадают**. В прод-логах не должны появляться полные payload и PII.
