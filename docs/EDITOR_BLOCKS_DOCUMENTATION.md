...
| `condition` | Выбор | 🔀 | #9C27B0 | Источник значения, значение для проверки, две исходящие ветки («Совпало», «Не совпало») |
...
**Назначение:** встроенная справка по блокам редактора без отдельного сайта документации.
