...
- `scripts/start-dev.ps1` (Windows, одна команда из корня репозитория)  
  ...
  - Освобождает порты **`8001` (основной backend)**, **`8002` (legacy / старый дефолт)**, **`5173`**, **`5174`**.  
...
- `scripts/stop-dev.ps1` — остановка процессов на портах **`8001`**, **`8002`**, **`5173`**, **`5174`**.
...
Чаще всего фронт запущен **`npm run dev`**, а бэкенд — **`uvicorn ... --port 8001`**, но переменная **`BOTFORG_BACKEND_PORT`** указывала на другой порт (раньше дефолт прокси был **8002**). Запросы шли не на ваш uvicorn → обрыв прокси или чужой процесс → **500**.
