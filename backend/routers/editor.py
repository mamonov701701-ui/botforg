...
def save_editor_state(template_id: int, state: Dict[str, Any], db: Session = Depends(get_db)):
    try:
        ...
        template_exists = db.query(BotTemplate).filter(BotTemplate.id == template_id).first()
        if not template_exists:
            logger.error(f"Template with id {template_id} not found in bot_templates table")
