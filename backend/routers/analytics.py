...
def analytics_templates(db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    templates = db.query(Template).filter(Template.user_id == current_user.id).all()
    result = []
    for tpl in templates:
        launches = db.query(BotUserState).filter(BotUserState.bot.has(template_id=tpl.id)).count()
        completions = db.query(BotUserState).filter(BotUserState.bot.has(template_id=tpl.id), BotUserState.current_node_id == None).count()
        payments_count = db.query(Payment).filter(Payment.template_id == tpl.id, Payment.status == 'paid').count()
        payments_sum = db.query(func.sum(Payment.amount)).filter(Payment.template_id == tpl.id, Payment.status == 'paid').scalar() or 0
        result.append({
            'id': tpl.id,
            'name': tpl.name,
            'launches': launches,
...
def analytics_template(id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    tpl = db.query(Template).filter(Template.id == id, Template.user_id == current_user.id).first()
    if not tpl:
        raise HTTPException(status_code=404, detail='Шаблон не найден')
    launches = db.query(BotUserState).filter(BotUserState.bot.has(template_id=tpl.id)).count()
    completions = db.query(BotUserState).filter(BotUserState.bot.has(template_id=tpl.id), BotUserState.current_node_id == None).count()
    payments = db.query(Payment).filter(Payment.template_id == tpl.id, Payment.status == 'paid').all()
    payments_sum = sum(p.amount for p in payments)
    unique_users = db.query(BotUserState.telegram_user_id).filter(BotUserState.bot.has(template_id=tpl.id)).distinct().count()
    # Среднее время прохождения
    times = []
    for state in db.query(BotUserState).filter(BotUserState.bot.has(template_id=tpl.id)):
    ...
    for state in db.query(BotUserState).filter(BotUserState.bot.has(template_id=tpl.id)):
        if state.current_node_id:
            exit_nodes[state.current_node_id] = exit_nodes.get(state.current_node_id, 0) + 1
...
def analytics_payments(
    ...
    payments = q.all()
    return [
        {'id': p.id, 'amount': p.amount, 'currency': p.currency, 'status': p.status, 'created_at': p.created_at, 'bot_id': p.bot_id, 'template_id': p.template_id}
        for p in payments
    ]

@router.get('/analytics/stats/{template_id}')
def analytics_stats(template_id: int, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
