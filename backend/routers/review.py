...
def add_review(
    ...
    existing = (
        db.query(Review)
        .filter(Review.template_id == id, Review.user_id == user.id)
        .first()
    )
    if existing:
    ...
    avg = (
        db.query(func.avg(Review.rating)).filter(Review.template_id == id).scalar() or 0
    )
