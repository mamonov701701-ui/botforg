...
def create_comment(comment: CommentCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    db_comment = Comment(user_id=current_user.id, template_id=comment.template_id, content=comment.content)
    db.add(db_comment)
    db.commit()
...
def get_my_comments(db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
