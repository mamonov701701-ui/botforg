...
def create_rating(rating: RatingCreate, db: Session = Depends(get_db), current_user: UserModel = Depends(get_current_user)):
    ...
    db_rating = db.query(Rating).filter(
        ...
        Rating.template_id == rating.template_id
    ).first()
    if db_rating:
        ...
        return db_rating
    db_rating = Rating(user_id=current_user.id, template_id=rating.template_id, score=rating.score)
    db.add(db_rating)
    db.commit()
    db.refresh(db_rating)
    return db_rating
...
def get_template_average_rating(template_id: int, db: Session = Depends(get_db)):
    avg = db.query(func.avg(Rating.score)).filter(Rating.template_id == template_id).scalar()
