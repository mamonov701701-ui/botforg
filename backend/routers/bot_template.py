...
async def create_bot_template(
    ...
    existing_active = db.query(BotTemplate).filter(
        BotTemplate.bot_id == bot_template.bot_id,
        BotTemplate.is_active == True
    ).first()
    
    if existing_active:
        existing_active.is_active = False
        existing_active.updated_at = datetime.utcnow()
        logger.info(f"Deactivated existing bot template binding for bot {bot_template.bot_id}")
    
    # Создаем новую привязку
    db_bot_template = BotTemplate(
        bot_id=bot_template.bot_id,
        user_template_id=bot_template.user_template_id
    )
    ...
    logger.info(f"Bot template binding created: bot {bot_template.bot_id} -> template {bot_template.user_template_id} by user {current_user.email}")
...
async def get_bot_templates(
    ...
    bot_templates = db.query(BotTemplate).join(Bot).filter(
        Bot.owner_id == current_user.id
    ).all()
...
async def get_bot_template(
    ...
    bot_template = db.query(BotTemplate).join(Bot).filter(
        BotTemplate.id == bot_template_id,
        Bot.owner_id == current_user.id
    ).first()
...
async def update_bot_template(
    ...
    logger.info(f"Bot template binding updated: {bot_template_id} by user {current_user.email}")
    return bot_template
...
async def delete_bot_template(
    ...
    logger.info(f"Bot template binding deactivated: {bot_template_id} by user {current_user.email}")
