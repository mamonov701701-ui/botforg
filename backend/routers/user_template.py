...
async def create_user_template(
    ...
    if user_template.template_id:
        template = (
            db.query(Template).filter(Template.id == user_template.template_id).first()
        )
        if not template:
            raise HTTPException(
    ...
    logger.info(
        f"User template created: {db_user_template.title} by user {current_user.email}"
    )
...
async def get_user_templates(
    ...
    user_templates = (
        db.query(UserTemplate).filter(UserTemplate.owner_id == current_user.id).all()
    )
...
async def get_user_template(
    ...
    if not user_template:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User template not found"
        )
...
async def update_user_template(
    ...
    if user_template_update.template_id is not None:
        template = (
            db.query(Template)
            .filter(Template.id == user_template_update.template_id)
            .first()
        )
        if not template:
            raise HTTPException(
    ...
    logger.info(
        f"User template updated: {user_template.title} by user {current_user.email}"
    )
    return user_template
...
async def delete_user_template(
    ...
    logger.info(
        f"User template deactivated: {user_template.title} by user {current_user.email}"
    )
