...
def get_marketplace(
    ...
    for t in templates:
        avg_rating = (
            db.query(func.coalesce(func.avg(Rating.score), 0.0))
            .filter(Rating.template_id == t.id)
            .scalar()
        )
        ...
        if user_id:
            is_purchased = (
                db.query(Purchase)
                .filter(Purchase.user_id == user_id, Purchase.template_id == t.id)
                .first()
                is not None
            )
        result.append(
...
def get_marketplace_template(
    ...
    t = (
        db.query(Template)
        .filter(Template.id == template_id, Template.is_public == True)
        .first()
    )
    if not t:
        return None
    ...
    tags = [TagOut.from_orm(tag) for tag in t.tags]
    comments = [
        CommentOut.from_orm(c)
        for c in db.query(Comment).filter(Comment.template_id == t.id).all()
    ]
    user_id = getattr(current_user, "id", None)
    is_purchased = False
    if user_id:
...
def copy_template(
    ...
    tpl = (
        db.query(Template).filter(Template.id == id, Template.is_public == True).first()
    )
    if not tpl:
