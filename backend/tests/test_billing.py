...
def test_create_paid_message():
    ...
    client.patch("/billing/quota", json=quota_update, headers={"Authorization": auth_header})
...
def test_quota_limit_enforcement():
    ...
    res3 = client.post("/messages/", json=message_data3, headers={"Authorization": auth_header})
...
def test_register_message_billing():
    ...
    message_res = client.post("/messages/", json=message_data, headers={"Authorization": auth_header})
    ...
    res = client.post("/billing/message", json=billing_data, headers={"Authorization": auth_header})
    assert res.status_code == 200
    data = res.json()
...
def test_register_billing_for_other_user_forbidden():
    ...
    res = client.post("/billing/message", json=billing_data, headers={"Authorization": auth_header})
    assert res.status_code == 200  # Теперь всегда создает запись для текущего пользователя
    data = res.json()
...
def test_update_user_quota():
    ...
    res = client.patch("/billing/quota", json=quota_update, headers={"Authorization": auth_header})
    assert res.status_code == 200
    data = res.json()
...
def test_access_other_user_billing_forbidden():
    ...
    client.post("/messages/", json=message_data, headers={"Authorization": auth_header1})
...
def test_correct_pricing_calculation():
    ...
    client.post("/messages/", json=message_data1, headers={"Authorization": auth_header})
    ...
    client.post("/messages/", json=message_data2, headers={"Authorization": auth_header})
