...
def test_create_incoming_message():
    ...
    res = client.post("/messages/", json=message_data, headers={"Authorization": auth_header})
    assert res.status_code == 201
    data = res.json()
...
def test_get_messages():
    ...
    res1 = client.post("/messages/", json=message_data1, headers={"Authorization": auth_header})
    res2 = client.post("/messages/", json=message_data2, headers={"Authorization": auth_header})
...
def test_get_messages_with_filters():
    ...
    # Filter by direction
    res_filtered = client.get("/messages/?direction=incoming", headers={"Authorization": auth_header})
    ...
    # Filter by status
    res_filtered = client.get("/messages/?status=sent", headers={"Authorization": auth_header})
    assert res_filtered.status_code == 200
    data = res_filtered.json()
    assert data["total"] == 1
...
def test_get_message():
    ...
    res_create = client.post("/messages/", json=message_data, headers={"Authorization": auth_header})
    assert res_create.status_code == 201
    message_id = res_create.json()["id"]
    
    # Get the specific message
    res_get = client.get(f"/messages/{message_id}", headers={"Authorization": auth_header})
...
def test_update_message():
    ...
    res_update = client.patch(f"/messages/{message_id}", json=update_data, headers={"Authorization": auth_header})
...
def test_update_nonexistent_message():
    ...
    res = client.patch("/messages/999999", json=update_data, headers={"Authorization": auth_header})
    assert res.status_code == 404
    assert "Message not found" in res.json()["detail"]
...
def test_delete_message():
    ...
    # Delete the message
    res_delete = client.delete(f"/messages/{message_id}", headers={"Authorization": auth_header})
...
def test_access_other_user_message_forbidden():
    ...
    res_create = client.post("/messages/", json=message_data, headers={"Authorization": auth_header1})
    assert res_create.status_code == 201
    message_id = res_create.json()["id"]
    
    # User 1 can see their message
    res_get1 = client.get(f"/messages/{message_id}", headers={"Authorization": auth_header1})
    assert res_get1.status_code == 200
    
    # User 2 cannot see User 1's message
    res_get2 = client.get(f"/messages/{message_id}", headers={"Authorization": auth_header2})
