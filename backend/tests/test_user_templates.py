...
def create_test_template(client: TestClient, auth_header: str) -> int:
    ...
    res = client.post("/templates/", json=template_data, headers={"Authorization": auth_header})
...
def test_get_user_templates_list():
    ...
    res1 = client.post("/user-templates/", json=user_template_data1, headers={"Authorization": auth_header})
    res2 = client.post("/user-templates/", json=user_template_data2, headers={"Authorization": auth_header})
...
def test_get_user_template():
    ...
    res_create = client.post("/user-templates/", json=user_template_data, headers={"Authorization": auth_header})
    assert res_create.status_code == 201
    template_id = res_create.json()["id"]
    
    # Get the specific template
    res_get = client.get(f"/user-templates/{template_id}", headers={"Authorization": auth_header})
...
def test_update_user_template():
    ...
    res_update = client.patch(f"/user-templates/{template_id}", json=update_data, headers={"Authorization": auth_header})
...
def test_update_nonexistent_user_template():
    ...
    res = client.patch("/user-templates/999999", json=update_data, headers={"Authorization": auth_header})
    assert res.status_code == 404
    assert "User template not found" in res.json()["detail"]
...
def test_delete_user_template():
    ...
    res_delete = client.delete(f"/user-templates/{template_id}", headers={"Authorization": auth_header})
...
def test_delete_nonexistent_user_template():
    ...
    res = client.delete("/user-templates/999999", headers={"Authorization": auth_header})
    assert res.status_code == 404
    assert "User template not found" in res.json()["detail"]
...
def test_user_template_isolation():
    ...
    res_create = client.post("/user-templates/", json=user_template_data, headers={"Authorization": auth_header1})
    assert res_create.status_code == 201
    template_id = res_create.json()["id"]
    
    # User 1 can see their template
    res_get1 = client.get(f"/user-templates/{template_id}", headers={"Authorization": auth_header1})
    assert res_get1.status_code == 200
    
    # User 2 cannot see User 1's template
    res_get2 = client.get(f"/user-templates/{template_id}", headers={"Authorization": auth_header2})
